              ...
              return result.catch(function(error){
                ...
                return new Promise(function (resolve) {
                  setTimeout(function () {
                    ...
                    fetch('/?sections=manifest')
                    .then(res => res.json())
                    .then(res => {
                      const file = chunkId + '.js'
                      const Manifest = JSON.parse(new DOMParser().parseFromString(res.manifest, 'text/html').querySelector('#shopify-section-manifest').textContent)
